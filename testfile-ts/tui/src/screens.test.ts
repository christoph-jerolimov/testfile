...
test("every TUI page renders exactly the committed screenshot", async () => {
  ...
  for (const screen of screens) {
    ...
    try {
      ...
      assert.fail(
        `no committed screenshot for "${screen.name}" - run: npm run screens:update --workspace @testfile.dev/tui`,
      );
    }
    assert.equal(
      screen.frame,
      committed,
      `the "${screen.name}" screen changed - if intended, run: npm run screens:update --workspace @testfile.dev/tui`,
    );
  }
});
