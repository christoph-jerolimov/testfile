...
export function registerInspect(program: Command): void {
  ...
  inspectCommand
    ...
    .option(
      "--json [file]",
      "write the run's record as JSON, to a file or (without a value) stdout",
    )
    .description("Show one recorded run")
    .action(
      (id: string, path: string, options: { log?: string | boolean; json?: string | boolean }) => {
        try {
          const history = loadedHistory(path);
          const run = findRun(history, id);

          if (wantsJson(options.json)) {
            // --log is a stream of raw text; the two cannot share one output.
            if (options.log !== undefined) {
              throw new Error("--log prints raw text, so it cannot be combined with --json");
            }
            writeJson(run, options.json);
            return;
          }
